 ...
 */
public class YHBaseTableSource implements BatchTableSource<Row>, ProjectableTableSource<Row>, StreamTableSource<Row>, LookupableTableSource<Row> {
     ...
     */
    public YHBaseTableSource(Configuration conf, String tableName) {
        this(conf, tableName, new HBaseTableSchema(), null);
    }

    public YHBaseTableSource(Configuration conf, String tableName, HBaseTableSchema hbaseSchema, int[] projectFields) {
        this.conf = conf;
    ...
    public DataSet<Row> getDataSet(ExecutionEnvironment execEnv) {
        HBaseTableSchema projectedSchema = hbaseSchema.getProjectedHBaseTableSchema(projectFields);
        return execEnv
            .createInput(new YHBaseRowInputFormat(conf, tableName, projectedSchema), getReturnType())
            .name(explainSource());
    }

    @Override
    public YHBaseTableSource projectFields(int[] fields) {
        return new YHBaseTableSource(this.conf, tableName, hbaseSchema, fields);
    }

    @Override
    public String explainSource() {
        return "YHBaseTableSource[schema=" + Arrays.toString(getTableSchema().getFieldNames())
    ...
    public DataStream<Row> getDataStream(StreamExecutionEnvironment execEnv) {
        HBaseTableSchema projectedSchema = hbaseSchema.getProjectedHBaseTableSchema(projectFields);
        return execEnv
            .createInput(new YHBaseRowInputFormat(this.conf, tableName, projectedSchema), getReturnType())
            .name(explainSource());
    }
